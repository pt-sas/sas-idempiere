...
public class LiveModelFactory implements IModelFactory {

	private static CCache<String,Class<?>> s_classCache = new CCache<String,Class<?>>(null, "PO_Class", 20, false);
	private final static CLogger s_log = CLogger.getCLogger(LiveModelFactory.class);
	...
	private static final String[]	s_packages = new String[] {
		...
		"org.adempiere.model",
		"org.idempiere.component","org.eevolution.model", "org.doubleclick.model"
	};
	...
	public Class<?> getClass(String tableName) {
		s_log.fine("LIBERO HR MODEL FACTORY -- Getting class for table: " + tableName);
//		Not supported
		...
		//@win add
		clazz = getPOclass("org.eevolution.model.X_" + tableName, tableName);
		if (clazz != null)
		{
			s_classCache.put(tableName, clazz);
			return clazz;
		}
		clazz = getPOclass("org.doubleclick.model.X_" + tableName, tableName);
		if (clazz != null)
		{
			s_classCache.put(tableName, clazz);
			return clazz;
