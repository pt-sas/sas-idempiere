...
public class FinReport extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			if (para[i].getParameter() == null)
				...
				p_OrgTrx_ID = ((BigDecimal)para[i].getParameter()).intValue();
			else
	...
	private void insertLineDetail()
	{
		...
		if (!listSourceNoTrx) {
			...
			StringBuilder sql = new StringBuilder("DELETE FROM T_Report WHERE ABS(LevelNo)<>0")
				...
				.append(" AND Col_20 IS NULL AND Col_21 IS NULL AND Col_22 IS NULL AND Col_23 IS NULL AND Col_24 IS NULL AND Col_25 IS NULL AND Col_26 IS NULL AND Col_27 IS NULL AND Col_28 IS NULL AND Col_29 IS NULL")
				.append(" AND Col_30 IS NULL AND Col_31 IS NULL AND Col_32 IS NULL AND Col_33 IS NULL AND Col_34 IS NULL AND Col_35 IS NULL AND Col_36 IS NULL AND Col_37 IS NULL AND Col_38 IS NULL AND Col_39 IS NULL")
				.append(" AND Col_40 IS NULL AND Col_41 IS NULL AND Col_42 IS NULL AND Col_43 IS NULL AND Col_44 IS NULL AND Col_45 IS NULL AND Col_46 IS NULL AND Col_47 IS NULL AND Col_48 IS NULL AND Col_49 IS NULL")
				.append(" AND Col_50 IS NULL AND Col_51 IS NULL AND Col_52 IS NULL AND Col_53 IS NULL AND Col_54 IS NULL AND Col_55 IS NULL AND Col_56 IS NULL AND Col_57 IS NULL AND Col_58 IS NULL AND Col_59 IS NULL")
				.append(" AND Col_60 IS NULL AND Col_61 IS NULL AND Col_62 IS NULL AND Col_63 IS NULL AND Col_64 IS NULL AND Col_65 IS NULL AND Col_66 IS NULL AND Col_67 IS NULL AND Col_68 IS NULL AND Col_69 IS NULL")
				.append(" AND Col_70 IS NULL AND Col_71 IS NULL AND Col_72 IS NULL AND Col_73 IS NULL AND Col_74 IS NULL AND Col_75 IS NULL AND Col_76 IS NULL AND Col_77 IS NULL AND Col_78 IS NULL AND Col_79 IS NULL")
				.append(" AND Col_80 IS NULL AND Col_81 IS NULL AND Col_82 IS NULL AND Col_83 IS NULL AND Col_84 IS NULL AND Col_85 IS NULL AND Col_86 IS NULL AND Col_87 IS NULL AND Col_88 IS NULL AND Col_89 IS NULL")
				.append(" AND Col_90 IS NULL AND Col_91 IS NULL AND Col_92 IS NULL AND Col_93 IS NULL AND Col_94 IS NULL AND Col_95 IS NULL AND Col_96 IS NULL AND Col_97 IS NULL AND Col_98 IS NULL AND Col_99 IS NULL")
				; 
			int no = DB.executeUpdate(sql.toString(), get_TrxName());
	...
	private void insertLineSource (int line)
	{
		...
		for (int col = 0; col < m_columns.length; col++)
		{
			...
			// ISY-178
			int orgTrxID = m_columns[col].get_ValueAsInt("AD_OrgTrx_ID");
			if(orgTrxID > 0){
				select.append(" AND AD_OrgTrx_ID=").append(orgTrxID);
			}
			...
			if (s != null && s.length() > 0)
				select.append(" AND ").append(s);
			//  Line Where
			s = m_lines[line].getWhereClause(p_PA_Hierarchy_ID);	//	(sources, posting type)
			if (s != null && s.length() > 0)
				select.append(" AND ").append(s);
