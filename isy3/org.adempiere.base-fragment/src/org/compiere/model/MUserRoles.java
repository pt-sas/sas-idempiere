...
public class MUserRoles extends X_AD_User_Roles
{
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		int maxUserRole = AWNSysConfig.getIntValue(AWNSysConfig.AWN_MAX_USERROLE, 0);
		if(maxUserRole>0){
			StringBuffer sb = new StringBuffer();
			sb.append("SELECT COUNT(DISTINCT AD_User_ID) ");
			sb.append("FROM AD_User_Roles ");
			sb.append("WHERE AD_Client_ID = ? ");
			int countUser = DB.getSQLValue(null, sb.toString(), getAD_Client_ID());
			if(countUser > maxUserRole){
				throw new AdempiereException("Maximum user limit exceeded: " + maxUserRole) ;
			}
		}
