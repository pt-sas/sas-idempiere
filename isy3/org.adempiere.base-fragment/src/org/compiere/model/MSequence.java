...
public class MSequence extends X_AD_Sequence
{
	...
	public static String getDocumentNoFromSeq(MSequence seq, String trxName, PO po) {
		...
		if (isStartNewYear || isUseOrgLevel) {
			selectSQL = "SELECT y.CurrentNext, s.CurrentNextSys, y.MaxValue "
			...
			selectSQL = "SELECT s.CurrentNext, s.CurrentNextSys, s.MaxValue "
					+ "FROM AD_Sequence s "
		...
		try
		{
			...
			if (rs.next())
			{
				...
				if(max > 0 && (adempiereSys ? rs.getInt(2) : rs.getInt(1)) > max){
					isExceedsMax = true;
		...
		catch (Exception e)
		{
			...
			if(isExceedsMax){
				throw new AdempiereException("DocumentNo exceeds maximum value", e);
