...
import org.compiere.model.MOrder.StockInfo;
 ...
 */
public class MOrderAWN extends MOrder implements DocAction
{
	/**
	 * 
	 */
	 ...
	 *	@return Order
	 */
	public static MOrderAWN copyFrom (MOrderAWN from, Timestamp dateDoc, 
		int C_DocTypeTarget_ID, boolean isSOTrx, boolean counter, boolean copyASI, 
		String trxName)
	{
		MOrderAWN to = new MOrderAWN (from.getCtx(), 0, trxName);
	 ...
	 */
	public MOrderAWN(Properties ctx, int C_Order_ID, String trxName)
	{
	 ...
	 */
	public MOrderAWN (MProject project, boolean IsSOTrx, String DocSubTypeSO)
	{
	 ...
	 */
	public MOrderAWN (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 */
	public int copyLinesFrom (MOrderAWN otherOrder, boolean counter, boolean copyASI)
	{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (   is_ValueChanged("AD_Org_ID")
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_C_BPartner_ID)
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_C_BPartner_Location_ID)
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_DateOrdered)
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_DatePromised)
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_M_Warehouse_ID)
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_M_Shipper_ID)
		    || is_ValueChanged(MOrderAWN.COLUMNNAME_C_Currency_ID)) {
			MOrderLine[] lines = getLines();
			for (MOrderLine line : lines) {
				if (is_ValueChanged("AD_Org_ID"))
					line.setAD_Org_ID(getAD_Org_ID());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_C_BPartner_ID))
					line.setC_BPartner_ID(getC_BPartner_ID());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_C_BPartner_Location_ID))
					line.setC_BPartner_Location_ID(getC_BPartner_Location_ID());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_DateOrdered))
					line.setDateOrdered(getDateOrdered());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_DatePromised))
					line.setDatePromised(getDatePromised());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_M_Warehouse_ID))
					line.setM_Warehouse_ID(getM_Warehouse_ID());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_M_Shipper_ID))
					line.setM_Shipper_ID(getM_Shipper_ID());
				if (is_ValueChanged(MOrderAWN.COLUMNNAME_C_Currency_ID))
	...
	{
		System.out.println("Extension point of IModelFactory");
		log.info(toString());
		...
		// Bug 1564431
		if (getDeliveryRule() != null && getDeliveryRule().equals(MOrderAWN.DELIVERYRULE_CompleteOrder)) 
		{
			for (int i = 0; i < lines.length; i++) 
			{
				MOrderLine line = lines[i];
	...
	public String completeIt()
	{
		...
		//	Counter Documents
		MOrderAWN counter = createCounterDoc();
		if (counter != null)
	...
	private String createPOSPayments() {
		...
		if (! this.isSOTrx())
			return null;
		if (! MOrderAWN.DocSubTypeSO_POS.equals(this.getC_DocType().getDocSubTypeSO()))
			return null;
		if (! MOrderAWN.PAYMENTRULE_MixedPOSPayment.equals(this.getPaymentRule()))
			return null;
	 ...
	 */
	private MOrderAWN createCounterDoc()
	{
		...
		//	Deep Copy
		MOrderAWN counter = copyFrom (this, getDateOrdered(), 
	...
	public boolean voidIt()
	{
		...
		MFactAcct.deleteEx(MOrderAWN.Table_ID, getC_Order_ID(), get_TrxName());
		setPosted(false);
	...
	public String reopenIt() {
		log.info(toString());
		if (!MOrderAWN.DOCSTATUS_Closed.equals(getDocStatus()))
		{
		...
		setDocStatus(MOrderAWN.DOCSTATUS_Completed);
		setDocAction(DOCACTION_Close);
