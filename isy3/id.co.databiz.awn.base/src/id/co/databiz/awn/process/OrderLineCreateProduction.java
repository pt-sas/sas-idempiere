...
//				log.log(Level.SEVERE, "Unknown Parameter: " + name);
	...
	protected String doIt () throws Exception
	{
		...
		// qty validation
		BigDecimal totalProductionQty = DB.getSQLValueBD(get_TrxName(), 
				"SELECT COALESCE(SUM(ProductionQty),0) FROM M_Production WHERE C_OrderLine_ID = ?", 
				p_C_OrderLine_ID);
		if(totalProductionQty.compareTo(line.getQtyOrdered()) >= 0)
			throw new IllegalArgumentException("Order has already been fulfilled: " + totalProductionQty);
		
		MProduction production = new MProduction( line );
		MProduct product = new MProduct (getCtx(), line.getM_Product_ID(), get_TrxName());
		
		//	ISY-162
		production.set_ValueOfColumn("C_DocType_ID", SystemID.DOCTYPE_MATERIAL_PRODUCTION);
		production.setM_Product_ID(line.getM_Product_ID());
		production.setProductionQty(line.getQtyOrdered().subtract(totalProductionQty));
