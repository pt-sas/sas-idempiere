 ...
 */
public class AgingR extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (log.isLoggable(Level.INFO)) log.info("StatementDate=" + p_StatementDate + ", IsSOTrx=" + p_IsSOTrx
			...
			+ ", IsListInvoices=" + p_IsListInvoices + ", SalesRep_ID=" + p_SalesRep_ID
		...
		if (p_C_Currency_ID == 0)
		{
			...
			{
				sql.append(" oi.GrandTotal, invoicePaidToDate(oi.C_Invoice_ID, oi.C_Currency_ID, 1,"+dateacct+") AS PaidAmt, invoiceOpenToDateR(oi.C_Invoice_ID,oi.C_InvoicePaySchedule_ID,"+dateacct+") AS OpenAmt ");			//	11..13
			}
		}
		else
		{
			...
			else
			{
				...
				.append(", currencyConvert(invoiceOpenToDateR(oi.C_Invoice_ID,oi.C_InvoicePaySchedule_ID,"+dateacct+")").append(s);  // 13
			}
		}
		sql.append(",oi.C_Activity_ID,oi.C_Campaign_ID,oi.C_Project_ID,oi.AD_Org_ID,oi.AD_OrgTrx_ID,oi.DateReceived ");	//	14..19
		if(p_ConversionType_ID>0 && p_StatementDate!=null){
			if(p_DateAcct){
				sql.append(", currencyConvert(invoiceOpenToDateR(oi.C_Invoice_ID,oi.C_InvoicePaySchedule_ID,"+dateacct+"),oi.C_Currency_ID," + SystemID.CURRRENCY_IDR + ","+dateacct+","+p_ConversionType_ID+",oi.AD_Client_ID,oi.AD_Org_ID)");
			} else {
		...
		if (!p_DateAcct)//FR 1933937
		{
			sql.append(" FROM RV_OpenItemR oi");
		}
		else
		{
			sql.append(" FROM RV_OpenItemToDateR oi");
		}
		...
		if (p_DateAcct)//FR 1933937
		{
			sql.append(" AND invoiceOpenToDateR(oi.C_Invoice_ID,oi.C_InvoicePaySchedule_ID,"+dateacct+") <> 0 ");
		}
		sql.append(" AND oi.DateReceived IS NOT NULL ");
		...
		try
		{
			...
			while (rs.next())
			{
				...
				BigDecimal OpenAmtReval = rs.getBigDecimal(20);
