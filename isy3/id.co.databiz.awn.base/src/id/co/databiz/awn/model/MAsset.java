	...
	//implements MAssetType.Model //commented by @win
{
	 ...
	 */
	public static Collection<org.compiere.model.MAsset> forASI(Properties ctx, int M_Product_ID, int M_ASI_ID)
	{
	...
	public MAsset(MInOut mInOut, MInOutLine sLine, int deliveryCount) {
		...
		// ISY-7
		setName(mInOut.getC_BPartner().getValue() + "_" + sLine.getM_Product().getValue());
		...
		if (sLine.getM_AttributeSetInstance_ID() > 0) {
			if (sLine.getM_AttributeSetInstance().getSerNo() != null) {
				setSerNo(sLine.getM_AttributeSetInstance().getSerNo());
			}
			if (sLine.getM_AttributeSetInstance().getLot() != null) {
				setLot(sLine.getM_AttributeSetInstance().getLot());
			}
		}
	}
