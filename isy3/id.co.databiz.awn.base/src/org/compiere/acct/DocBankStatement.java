 ...
 *
 */
public class DocBankStatement extends Doc_BankStatement
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public DocBankStatement (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if ((!as.isPostIfClearingEqual()) && acct_bank_asset.equals(acct_bank_in_transit)) {
				...
				if (amt_stmt_minus_trx.compareTo(Env.ZERO) != 0) {
					...
					if (fl != null && payment.getC_Payment_ID() > 0)
						fl.setAD_OrgTrx_ID(payment.getAD_OrgTrx_ID());
					else if (fl != null)
						fl.setAD_OrgTrx_ID(getAD_OrgTrx_ID());
				}
