...
//		super (as, MAllocationHdr.class, rs, DOCTYPE_Allocation, trxName);
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			BigDecimal allocationPaymentAccounted = Env.ZERO;
			...
			if (invoice == null)
			{
				...
				if (fl != null){
					allocationPaymentAccounted = fl.getAcctBalance();
					allocationPaymentSource = fl.getSourceBalance();
				}
			}
			//	Sales Invoice
			else if (invoice.isSOTrx())
			{
				...
				MAccount acct_receivable = getInvoiceAcct(as, invoice.getC_Invoice_ID(), true);
				...
				if ((!as.isPostIfClearingEqual()) && acct_unallocated_cash != null && acct_unallocated_cash.equals(acct_receivable) && (!isInterOrg)) {
					...
					allocationPaymentSource = allocationSource;
					...
					if (line.getC_Payment_ID() != 0)
					{
						...
						if (fl != null && payment != null){
							...
							allocationPaymentAccounted = fl.getAcctBalance().negate();
							allocationPaymentSource = fl.getSourceBalance().negate();
						}
					}
					else if (line.getC_CashLine_ID() != 0)
					{
						...
						if (fl != null && cashLine.get_ID() != 0){
							...
							fl.setAD_Org_ID(invoice.getAD_OrgTrx_ID());
						}
					}
				...
				if (as.isAccrual())
				{
					bpAcct = getInvoiceAcct(as, invoice.getC_Invoice_ID(),true);
			...
			else
			{
				...
				MAccount acct_liability = getInvoiceAcct(as, invoice.getC_Invoice_ID(),false);
				...
				if (as.isAccrual())
				{
					bpAcct = getInvoiceAcct(as, invoice.getC_Invoice_ID(),false);
					fl = fact.createLine (line, bpAcct,
			...
			// ISY-121 Ambil dari SYNC-11
			if(payment!=null){
				BigDecimal paymentRate = MConversionRate.getRate(payment.getC_Currency_ID(), MClient.get(getCtx()).getC_Currency_ID(), payment.getDateTrx(), payment.getC_ConversionType_ID(), payment.getAD_Client_ID(), payment.getAD_Org_ID());			
				MAllocationHdr allocation = (MAllocationHdr) getPO();
				BigDecimal allocationRate = MConversionRate.getRate(allocation.getC_Currency_ID(), MClient.get(getCtx()).getC_Currency_ID(), allocation.getDateTrx(), payment.getC_ConversionType_ID(), allocation.getAD_Client_ID(), allocation.getAD_Org_ID());
				if (paymentRate == null) {
					throw new AdempiereException("Rate - not found - CurFrom=" + payment.getC_Currency().getISO_Code() 
							  + ", CurTo=" + MCurrency.getISO_Code(getCtx(),MClient.get(getCtx()).getC_Currency_ID())
							  + ", " + payment.getDateTrx() 
							  + ", Type=" + payment.getC_ConversionType().getName() 
							  + ", Client=" + payment.getAD_Client_ID() 
							  + ", Org=" + payment.getAD_Org_ID());
				}
				if (allocationRate == null) {
					throw new AdempiereException("Rate - not found - CurFrom=" + allocation.getC_Currency().getISO_Code() 
							  + ", CurTo=" + MCurrency.getISO_Code(getCtx(),MClient.get(getCtx()).getC_Currency_ID())
							  + ", " + allocation.getDateTrx() 
							  + ", Type=" + payment.getC_ConversionType().getName() 
							  + ", Client=" + allocation.getAD_Client_ID() 
							  + ", Org=" + allocation.getAD_Org_ID());
				}
				if (payment != null
						&& (getC_Currency_ID() != as.getC_Currency_ID()			//	payment allocation in foreign currency
							|| getC_Currency_ID() != payment.getC_Currency_ID()) //	allocation <> payment currency
						&& payment.getReversal_ID()==0 && paymentRate.compareTo(allocationRate)!=0)	
				{
					pAcct = getPaymentAcct(as, payment.get_ID());
					p_Error = createRealizedGainLoss (line, as, fact, pAcct, payment, 
						allocationPaymentSource, allocationPaymentAccounted);
					if (p_Error != null)
						return null;
				}
			}
		...
		BigDecimal allocationSource)
	{
		...
		DocInvoice docInvoice = (DocInvoice)Doc.get(as,
	...
	private MAccount getInvoiceAcct (MAcctSchema as, int C_Invoice_ID, boolean IsSoTrx){
		int acctType = IsSoTrx ? Doc.ACCTTYPE_C_Receivable : Doc.ACCTTYPE_V_Liability;		
		if(C_Invoice_ID>0){
			MInvoice invoice = new MInvoice(getCtx(), C_Invoice_ID, getTrxName());
			...
			if(IsSoTrx){
				if(docTypeCustom.isAROther()){
					acctType = Doc.ACCTTYPE_C_Receivable_Services;
				} else {
					acctType = Doc.ACCTTYPE_C_Receivable;
				}
			} else {
				if(docTypeCustom.isAROther()){
					acctType = Doc.ACCTTYPE_V_Liability_Services;
				} else {
					acctType = Doc.ACCTTYPE_V_Liability;
				}
			}
		}
		...
		MInvoice invoice, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT ")
			.append(invoice.isSOTrx()
					? "SUM(AmtSourceDr+AmtSourceCr), SUM(AmtAcctDr+AmtAcctCr)"	//	so 
					: "SUM(AmtSourceCr+AmtSourceDr), SUM(AmtAcctCr+AmtAcctDr)")	//	po
			.append(" FROM Fact_Acct ")
			.append("WHERE AD_Table_ID=318 AND Record_ID=?")	//	Invoice
			.append(" AND C_AcctSchema_ID=?")
			.append(" AND PostingType='A'")
			.append(" AND C_Tax_ID IS NULL AND (AmtSourceDr+AmtSourceCr) <> 0"); // find header doc only, avoid tax & gain/loss
		...
		if (invoice.isSOTrx())
		{
			...
			FactLine flx = fact.createLine (line, acct, 
				as.getC_Currency_ID(), acctDifference.negate());
			flx.setDescription(description.toString());
			flx.setAD_Org_ID(invoice.getAD_Org_ID());
			flx.setAD_OrgTrx_ID(invoice.getAD_OrgTrx_ID());
			flx.setC_BPartner_ID(invoice.getC_BPartner_ID());
		...
		else
		{
			...
			FactLine flx = fact.createLine (line, loss, gain, 
				as.getC_Currency_ID(), acctDifference.negate());
			flx.setAD_Org_ID(invoice.getAD_Org_ID());
			flx.setAD_OrgTrx_ID(invoice.getAD_OrgTrx_ID());
			flx.setC_BPartner_ID(invoice.getC_BPartner_ID());
		...
		MPayment payment, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		//
		String sql = "SELECT " 
			+ (payment.getC_DocType().getDocBaseType().equals(Doc.DOCTYPE_ARReceipt) 
				? "SUM(AmtSourceDr), SUM(AmtAcctDr)"	//	so 
				: "SUM(AmtSourceCr), SUM(AmtAcctCr)")	//	po
			+ " FROM Fact_Acct "
			+ "WHERE AD_Table_ID=335 AND Record_ID=?"	//	Payment
			+ " AND C_AcctSchema_ID=?"
			+ " AND PostingType='A'";
			//AND C_Currency_ID=102
		PreparedStatement pstmt = null;
		...
		//
		String description = "Payment=(" + payment.getC_Currency_ID() + ")" + paymentSource + "/" + paymentAccounted
		...
		if (getC_Currency_ID() != payment.getC_Currency_ID())
		{
			...
			String d2 = "Allocation=(" + getC_Currency_ID() + ")" + allocationSource 
				+ "->(" + payment.getC_Currency_ID() + ")" + allocationSourceNew;
			log.fine(d2);
			description += " - " + d2;
			allocationSource = allocationSourceNew;
		}
		...
		BigDecimal acctDifference = null;	//	gain is negative
		//	Full Payment in currency
		if (allocationSource.compareTo(paymentSource) == 0)
		{
			acctDifference = paymentAccounted.subtract(allocationAccounted);	//	gain is negative
		...
		else	//	partial or MC
		{
			...
			//	Reduce Orig Invoice Accounted
			paymentAccounted = paymentAccounted.multiply(new BigDecimal(multiplier));
			//	Difference based on percentage of Orig Invoice
			acctDifference = paymentAccounted.subtract(allocationAccounted);	//	gain is negative
			//	ignore Tolerance
			if (acctDifference.abs().compareTo(TOLERANCE) < 0)
		...
		MAccount loss = MAccount.get (as.getCtx(), as.getAcctSchemaDefault().getRealizedLoss_Acct());
		//		
		if (payment.getC_DocType().getDocBaseType().equals(Doc.DOCTYPE_APPayment))
		{
			FactLine fl = fact.createLine (line, acct, 
					as.getC_Currency_ID(), acctDifference);
			...
			FactLine flx = fact.createLine (line, loss, gain, 
				as.getC_Currency_ID(), acctDifference.negate());
			flx.setDescription(description);
			flx.setAD_Org_ID(payment.getAD_Org_ID());
			flx.setAD_OrgTrx_ID(payment.getAD_OrgTrx_ID());
			flx.setC_BPartner_ID(payment.getC_BPartner_ID());
		}
		else
		{
			FactLine fl = fact.createLine (line, acct,
