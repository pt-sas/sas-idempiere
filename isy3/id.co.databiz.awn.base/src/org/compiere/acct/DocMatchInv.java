...
//		MMatchInv matchInv = (MMatchInv)getPO();
		...
		**/
		...
		if (as.isAccrual() && as.isCreatePOCommitment())
		{
			...
			for (int i = 0; i < commitments.length; i++)
			{
				...
				{
					this.p_Error = "Different Currencies of Order Lines";
					s_log.log(Level.SEVERE, this.p_Error);
					return null;
				}
				...
				//	Account
				MAccount expenseOrder = line.getAccount(ProductCost.ACCTTYPE_P_Expense, as);
				//	ISY-281
				if(m_matchInv.getReversal_ID() > 0)
					fl = fact.createLine (line, expenseOrder,
			...
			//	Offset
			MAccount offset = this.getAccount(ACCTTYPE_CommitmentOffset, as);
			if (offset == null)
			{
	...
	private DocLine[] getCommitmentsOrder(Doc doc, BigDecimal maxQty, int C_InvoiceLine_ID)
	{
		int precision = -1;
		//
