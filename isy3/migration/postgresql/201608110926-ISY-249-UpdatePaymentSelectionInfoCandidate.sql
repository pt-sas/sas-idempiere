...
--  
UPDATE AD_InfoWindow SET WhereClause='i.IsActive=''Y'' AND i.DocBaseType IN (''API'',''APC'') AND i.IsPaid=''N'' AND NOT EXISTS (SELECT * FROM C_PaymentAllocate pa WHERE pa.C_Invoice_ID=i.C_Invoice_ID) AND i.DocStatus IN (''CO'',''CL'')',Updated=TO_TIMESTAMP('2016-08-09 18:07:39','YYYY-MM-DD HH24:MI:SS'),UpdatedBy=100 WHERE AD_InfoWindow_ID=550006
;
