...
AND NOT EXISTS (SELECT 1 FROM C_PaymentAllocate pa INNER JOIN C_Payment p ON (p.C_Payment_ID = pa.C_Payment_ID) WHERE pa.C_Invoice_ID = C_Invoice.C_Invoice_ID AND p.DocStatus IN (''DR'',''IP''))',Updated=TO_TIMESTAMP('2016-07-22 16:45:15','YYYY-MM-DD HH24:MI:SS'),UpdatedBy=100 WHERE AD_Val_Rule_ID=550123
;
