...
public class InvoiceGenerateRMA extends SvrProcess
{
    ...
    private MInvoice createInvoice(MRMA rma)
    {
        ...
        if (rma.get_ValueAsInt("AD_OrgTrx_ID") > 0) {
        	invoice.setAD_OrgTrx_ID(rma.get_ValueAsInt("AD_OrgTrx_ID"));
        }
        if (rma.get_ValueAsInt("C_BPartner_Location_ID") > 0) {
        	invoice.setC_BPartner_Location_ID(rma.get_ValueAsInt("C_BPartner_Location_ID"));
        }
        ...
        invoice.setC_Activity_ID(0);
        invoice.setC_Campaign_ID(0);
        invoice.setUser1_ID(0);
        invoice.setUser2_ID(0);
        if (!invoice.save())
        {
    ...
    private MInvoiceLine[] createInvoiceLines(MRMA rma, MInvoice invoice)
    {
        ...
        for (MRMALine rmaLine : rmaLines)
        {
            ...
            // SAS-197 set price list to prevent zero price get overriden in before save
            invLine.setPrice();
            invLine.setPrice(rmaLine.getAmt());
            invLine.setC_Project_ID(0);
            invLine.setC_Activity_ID(0);
            invLine.setC_Campaign_ID(0);
            if (!invLine.save())
            {
