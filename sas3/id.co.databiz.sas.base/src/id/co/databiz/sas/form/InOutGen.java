...
public class InOutGen extends GenForm
{
	...
	public void configureMiniTable(IMiniTable miniTable)
	{
		...
		miniTable.addColumn("PriorityRule");
		//
		...
		miniTable.setColumnClass(i++, String.class, true, Msg.translate(Env.getCtx(), "PriorityRule"));
		//
		miniTable.autoSize();
	}
	...
	//  Create SQL
        StringBuilder sql = new StringBuilder(
            "SELECT ic.C_Order_ID, o.Name, dt.Name, ic.DocumentNo, bp.Name, ic.DateOrdered, ic.TotalLines, rl.Name, bpl.Name "
	    	// use C_Order instead of M_InOut_Candidate_v for access purposes, will be replaced later
            + "FROM C_Order ic "
            + "JOIN C_Order ox ON (ox.C_Order_ID=ic.C_Order_ID) "
            + "JOIN AD_Org o ON (o.AD_Org_ID=ic.AD_Org_ID) "
            + "JOIN C_BPartner bp ON (bp.C_BPartner_ID=ic.C_BPartner_ID) "
            + "JOIN C_BPartner_Location bpl ON (bpl.C_BPartner_Location_ID=ox.Bill_Location_ID) "
            + "JOIN C_DocType dt ON (dt.C_DocType_ID=ic.C_DocType_ID) "
            + "LEFT JOIN AD_Ref_List rl ON (rl.AD_Reference_ID = 154 AND rl.Value = ox.PriorityRule) "
        ...
        sql.append(" ORDER BY ox.PriorityRule, ox.DatePromised");
	...
	private String getRMASql()
	{
	    ...
	    sql.append("SELECT rma.M_RMA_ID, org.Name, dt.Name, rma.DocumentNo, bp.Name, rma.DateOrdered, rma.Amt, NULL, bpl.Name ");
	...
	public void executeQuery(KeyNamePair docTypeKNPair, IMiniTable miniTable)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				miniTable.setValueAt(rs.getString(9), row, column++);              //  BPLocation
				...
				miniTable.setValueAt(rs.getString(8), row, column++);              //  PriorityRule
				//  prepare next
				row++;
			}
		}
