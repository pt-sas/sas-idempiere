...
public class GenerateKwitansi extends GenForm
{
	...
	public void configureMiniTable(IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(3, KeyNamePair.class, true, Msg.translate(Env.getCtx(), "C_DocType_ID"));
		...
		miniTable.setColumnClass(8, String.class, true, "Tax Invoice");
		miniTable.setColumnClass(9, Timestamp.class, true, Msg.translate(Env.getCtx(), "DueDate"));
		//
		miniTable.autoSize();
	}
	...
	private String getSQL()
	{
	    ...
	    sql.append("PARSE('AD_Org',i.AD_Org_ID,1), ");
	    sql.append("PARSE('AD_Org',i.AD_OrgTrx_ID,1), ");
//	    sql.append("PARSE('C_DocType',i.C_DocType_ID,1), ");
	    ...
	    sql.append("AND (dt.DocBaseType = 'APC' OR EXISTS (SELECT 1 FROM R_Request r WHERE r.C_Invoice_ID=i.C_Invoice_ID AND r.R_Status_ID=1000008 AND r.Processed='Y')) ");
	    sql.append("AND NOT EXISTS (SELECT 1 FROM R_Request r WHERE r.C_Invoice_ID=i.C_Invoice_ID AND r.R_Status_ID IN(1000011,1000012,1000015,1000021,1000022)) ");
        ...
        if (billingID != null) {
        	sql.append(" AND bp.Billing_ID IN (SELECT bpb.C_BPartner_ID FROM C_BPartner bpb ")
        	.append("INNER JOIN AD_User u ON (u.C_BPartner_ID = bpb.C_BPartner_ID) WHERE u.AD_User_ID = ")
        	.append(billingID)
        	.append(")");
        }
...
//				miniTable.setValueAt(rs.getString(4), row, 3);              //  DocType
				...
				miniTable.setValueAt(rs.getString(9), row, 8);              //  TaxInvoice
				miniTable.setValueAt(rs.getTimestamp(10), row, 9);           //  DueDate
				//  prepare next
				row++;
			}
			rs.close();
		    ...
		    try
			{
				...
				while (rs.next())
				{
					...
					if(kwitansi==null || !(Boolean)isConsolidate || m_bpartnerID!=bpartnerID  || m_docTypeID!=docTypeID){
						...
						MUser user = new Query(Env.getCtx(), MUser.Table_Name, "C_BPartner_ID = ? AND IsActive='Y'", null)
									.setParameters(new Object[]{bPartner.get_ValueAsInt("Billing_ID")})
									.first();
						if(user!=null)
							kwitansi.setAD_User_ID(user.getAD_User_ID());
						if(bPartner!=null)
							kwitansi.set_ValueOfColumn("Collector_ID", bPartner.get_ValueAsInt("Collector_ID"));
					}
	...
	public String calculateSelection(IMiniTable miniTable)
	{
		...
		for (int i = 0; i < rows; i++)
		{
			...
			if (id.isSelected())
			{
				...
				if (amt != null) {
					KeyNamePair docTypeKNP = (KeyNamePair)miniTable.getValueAt(i, 3);
