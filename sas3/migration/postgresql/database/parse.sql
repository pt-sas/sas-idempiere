...
BEGIN
  ...
  ELSIF UPPER (p_Table) = 'C_BPARTNER_LOCATION' THEN
    ...
    ELSEIF p_Type = 101 THEN
        ...
        LEFT OUTER JOIN C_Country cc ON cc.C_COuntry_ID = l.C_Country_ID
        WHERE bpl.C_BPartner_Location_ID = p_ID;
    ELSEIF p_Type = 102 THEN
        SELECT  (  CASE WHEN l.Address1 IS NOT NULL THEN l.Address1 || ' ' ELSE '' END
                || CASE WHEN l.Address2 IS NOT NULL THEN l.Address2 || ' ' ELSE '' END
                || CASE WHEN l.Address3 IS NOT NULL THEN l.Address3 || ' ' ELSE '' END
                || CASE WHEN l.Address4 IS NOT NULL THEN l.Address4 || ' ' ELSE '' END
                || CASE WHEN l.city IS NOT NULL THEN l.city || (CASE WHEN l.postal IS NOT NULL THEN ' ' || l.postal ELSE '' END) || ' ' ELSE '' END
                || cc.Name)
            INTO v_Return
        FROM C_BPartner_Location bpl
        INNER JOIN C_Location l ON l.C_Location_ID = bpl.C_Location_ID
