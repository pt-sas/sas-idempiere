...
public abstract class CreateFromInvoice extends CreateFrom
{
	...
	protected ArrayList<KeyNamePair> loadOrderData (int C_BPartner_ID, boolean forInvoice, boolean sameWarehouseOnly)
	{
		...
		sql = sql.append("ORDER BY o.DocumentNo");
		//
	...
	protected ArrayList<KeyNamePair> loadShipmentData (int C_BPartner_ID)
	{
		...
		StringBuffer sql = new StringBuffer("SELECT s.M_InOut_ID,").append(display)
			...
			sql.append(") ORDER BY s.DocumentNo");
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(8, BigDecimal.class, true);    //  8-Unit Price
		//  Table UI
		miniTable.autoSize();
	}
...
//						}
	...
	protected Vector<String> getOISColumnNames()
	{
	    ...
	    columnNames.add(Msg.getElement(Env.getCtx(), "PriceActual"));
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		StringBuilder sql = new StringBuilder("SELECT "
			...
			+ (isSOTrx ? "-l.QtyInvoiced" : "")
			+ "-COALESCE((SELECT SUM(il.QtyInvoiced) FROM C_InvoiceLine il JOIN C_Invoice i ON i.C_Invoice_ID = il.C_Invoice_ID WHERE l.C_OrderLine_ID=il.C_OrderLine_ID AND i.Processed='N'),0),"	//	1
			+ "CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END,"	//	2
			+ " l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),"			//	3..4
			+ " COALESCE(l.M_Product_ID,0),COALESCE(p.Value||'-'||p.Name,c.Name),po.VendorProductNo,"	//	5..7
			+ " l.C_OrderLine_ID,l.Line, "								//	8..9
			+ " CASE WHEN l.QtyOrdered = 0 THEN l.PriceActual ELSE l.LineNetAmt/l.QtyOrdered END AS PriceActual "				// 10
			+ "FROM C_OrderLine l"
		...
		try
		{
			...
			while (rs.next())
			{
				...
				BigDecimal unitPrice = rs.getBigDecimal(10);
				line.add(unitPrice);					//  8-UnitPrice
				data.add(line);
			}
		}
		catch (Exception e)
