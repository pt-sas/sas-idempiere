...
public class WCreateFromInvoiceUI extends CreateFromInvoice implements EventListener<Event>, ValueChangeListener
{
    ...
    protected Label amountLabel = new Label();
    protected Label amountValueLabel = new Label();
	...
	protected void zkInit() throws Exception
	{
        ...
        amountLabel.setText(Msg.translate(Env.getCtx(), "TotalLines"));
		...
		row = rows.newRow();
        row.appendChild(amountLabel.rightAlign());
        row.appendChild(amountValueLabel);
	...
	public void info(IMiniTable miniTable, IStatusBar statusBar)
	{
		...
		for (int i = 0; i < rows; i++)
		{
			if (((Boolean) model.getValueAt(i, 0)).booleanValue()) {
				qty = (BigDecimal) model.getValueAt(i, 1);
				unitPrice = (BigDecimal) model.getValueAt(i, 8);
				if (unitPrice != null) {
					totalAmount = totalAmount.add(qty.multiply(unitPrice));
				}
			}
		}
		setTotalAmountStatus(totalAmount);
	}   //  infoInvoice
	
	private void setTotalAmountStatus(BigDecimal totalAmount) {
		amountValueLabel.setText(decimalFormat.format(totalAmount.doubleValue()));
	}
	
	protected void configureMiniTable (IMiniTable miniTable)
	{
		miniTable.setColumnClass(0, Boolean.class, false);      //  0-Selection
		if (orderField.getSelectedItem() != null && orderField.getSelectedItem().getValue() != null) {
